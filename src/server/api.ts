...
// Removed unused AIAnalysis import
...
{"weight": 0.0, "calories": 0.0, "protein": 0.0, "fat": 0.0, "carbs": 0.0}`
      ...
      if (typeof openaiError === 'object' && openaiError !== null) {
        if ('status' in openaiError) console.error('--> OpenAI Error Status:', (openaiError as any).status);
        if ('code' in openaiError) console.error('--> OpenAI Error Code:', (openaiError as any).code);
        if ('message' in openaiError) console.error('--> OpenAI Error Message:', (openaiError as any).message);
      }
